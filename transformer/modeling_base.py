...
class BertPooler(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        pooled_output = self.dense(first_token_tensor) # batch * dim
...
class BertForMaskedLM_and_CasePrediction(BertPreTrainedModel):

    def __init__(self, config):
        super(BertForMaskedLM_and_CasePrediction, self).__init__(config)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None, labels=None):
        
        # for mask calculation
        
        sequence_output, _, pooled_output = self.bert(input_ids, token_type_ids, attention_mask)
        ...
        if labels is not None:
            ...
            loss_fct = CrossEntropyLoss(ignore_index = self.num_labels -1)
            case_loss = loss_fct(logits.view(-1, self.num_labels), labels.view(-1))

        total_loss = masked_lm_loss + case_loss
        case_predict_logits = logits
        
        return total_loss, masked_lm_loss, case_loss, case_predict_logits
    
    


class BertForCasePrediction(BertPreTrainedModel):
