...
class SIIMImageDataset(BaseImageDataset):
    def __init__(self, split="train", transform=None,
                 data_pct=0.01, phase="segmentation", imsize=512):
        ...
        print(self.phase, self.df.shape)
        ...
        if data_pct != 1 and split == "train":
            ...
            self.df,_ = train_test_split(self.df, test_size=(1-data_pct), random_state=42)
            self.df = self.df.reset_index(drop=True)
...
class RSNASegmentDataset(BaseImageDataset):
    ...
    def __getitem__(self, index):
        ...
        img_path = os.path.join(RSNA_IMG_DIR, filename)
        x = self.read_from_dicom(img_path)
