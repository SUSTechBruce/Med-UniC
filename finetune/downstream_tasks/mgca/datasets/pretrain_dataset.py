...
class MultimodalPretrainingDataset(data.Dataset):
    def __init__(self, split="train", transform=None, data_pct=1.0,
                 imsize=224, max_words=112, sent_num=3):
        super().__init__()
        ...
        csv_path = '/home/cheliu/phd_work/multimodal_cxr_data/Image-Text/MIMIC-CXR-meta_INDI_FIND_IMP_report.csv'
        img_path = '/home/cheliu/phd_work/multimodal_cxr_data/Image-Text/9W_MIMIC_train.npy'
        self.df = pd.read_csv(csv_path)
        self.img_npy = np.load(img_path)
    ...
    def get_caption(self, index):
        ...
        findings = self.df['FINDINGS'].iloc[index]
        impression = self.df['IMPRESSION'].iloc[index]
        sent = findings + impression
