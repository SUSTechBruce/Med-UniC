...
from med_unic_model import ResNet_CXRBert
import warnings
...
def cli_main():
    ...
    parser.add_argument("--model_name", type=str, default="CAMP_vit_only")
    parser.add_argument("--save_path_name", type=str, default="CAMP_vit_only")
    ...
    if args.model_name == 'CAMP_vit_only':
        print('loading CAMP model')
        args.path = osp.join(os.path.abspath('.'), "pretrain_models/vit_base_80/cross-lingual_multi-modal_encoder.pth")
...
#             print('shape: ', img_feat[:, 0].unsqueeze(1).contiguous().shape)
            return img_feat[:, 0].unsqueeze(1).contiguous()
    ...
    if args.path :
        ...
        if args.model_name == 'resnet50':
        ...
        elif args.model_name == 'CAMP_vit_only':
            ...
            print('loading pretrained large vit ok')
            args.backbone = ViT_encoder(model).to('cuda')
            
        elif args.model_name == 'CAMP':
            pass
