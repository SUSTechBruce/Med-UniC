...
class ImageEncoder(nn.Module):
    def __init__(self,
        ...
        if "vit" in model_name:
            ...
            checkpoint = osp.join(os.path.abspath('.'), "vit_base/pretrain_vit_base.pth")
            vit_cpt = torch.load(checkpoint, map_location=torch.device('cpu'))['model']
            msg = self.model.load_state_dict(vit_cpt, strict=False)
...
class BertEncoder(nn.Module):
    def __init__(self,
        ...
        self.bert_type = osp.join(os.path.abspath('.'), "baseline_models/clinical_bert")
