...
def preprocess_chexpert_5x200_data():
    df = pd.read_csv(CHEXPERT_fillter)
    df = df.fillna(0)
...
def preprocess_chexpert_data():
    ...
    paths = df_200['Path'].tolist()
    
    count = 0
    for path in paths:
        path = path.replace('CheXpert-v1.0/', '')
        path = os.path.join(CHEXPERT_DATA_DIR, path)
        if os.path.exists(path):
            print('yes')
        else:
            count += 1
            print(os.path.join(CHEXPERT_DATA_DIR, path))
    ...
    print('The missing df200 counts is: ', count)
    print('The missing training counts is: ', t_count)
