...
def save_pkl(df, pkl_path):
    ...
    for i, box in enumerate(bboxs_list):
        
        if box!=['None']:
            new_filenames.append(box)
            new_bboxs_list.append(filenames[i])
            
        if box==['None']:
            count += 1
    
    print('fuck out',count)
    assert len(new_filenames) == len(new_bboxs_list)
