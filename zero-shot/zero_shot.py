...
if __name__ == '__main__':
    ...
    cxr_filepath = osp.join(os.path.abspath('.'), "chexpert_test.h5")
    cxr_true_labels_path = osp.join(os.path.abspath('.'), "test_labels.csv")

    model_path = osp.join(os.path.abspath('.'), "Camp_models/no_cta/epoch80/cross-lingual_multi-modal_total.pth")
...
#                                           'Consolidation', 'Edema','Pleural Effusion']
    ...
    cxr_labels: List[str] = ['atelectasi','Cardiomegalia', 
                                          'consolidacion', 'Edem','derramepleural']

    # ---- TEMPLATES ----- # 
    # Define set of templates | see Figure 1 for more details  
    cxr_pair_template: Tuple[str] = ("Hallazgos consistentes con {}", "No {}")
...
#     cxr_pair_template: Tuple[str] = ("{}", "No hay {}")
    ...
    print('y_pred', y_pred)
    auc = roc_auc_score(test_true, y_pred, average='macro')

    print('auc', auc)

    # compute accuracy and f1
    # acc and f1 must be computed for each label separately then averaged
    acc_total, f1_total = {}, {}
    acc_mean = 0
    f1_mean = 0
