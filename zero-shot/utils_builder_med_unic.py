...
class ResNet_CXRBert(torch.nn.Module):
    def __init__(self, args):
        ...
        if self.args.from_scratch:
            ...
            print('loading from un_pretrain cxrbert ###################')

            self.tokenizer = BertTokenizer.from_pretrained(self.args.un_pretrain_model, do_lower_case=self.args.do_lower_case)

            self.config = AutoConfig.from_pretrained(self.args.un_pretrain_model + '/config.json', cache_dir=self.args.cache_dir)
            self.lm_model = BertModel.from_pretrained(
                args.un_pretrain_model + '/pytorch_model.bin',
                from_tf=bool(".ckpt" in self.args.un_pretrain_model + '/pytorch_model.bin'),
                config=self.config,
            ...
            self.config = AutoConfig.from_pretrained(self.args.model + '/config.json',
                                                     cache_dir=self.args.cache_dir)

            self.lm_model = BertModel.from_pretrained(
                args.model + '/pytorch_model.bin',
                from_tf=bool(".ckpt" in self.args.model + '/pytorch_model.bin'),
                config=self.config,
                cache_dir=self.args.cache_dir)


        ### start to freeze the model #############################################################
        if self.args.from_scratch:
            if self.args.freeze_layers != 0:
                self.freeze_model_layer_new(self.lm_model, self.args.freeze_layers)
        else:
            if self.args.freeze_layers != 0:
                self.freeze_model_layer(self.lm_model, self.args.freeze_layers)

        print('Free model layer ################# : ', self.args.freeze_layers)
    ...
    def forward(self, img1, img2, domain_true, input_ids, attention_mask, unified_train=True, text_aug=0):
        ...
        clip_loss, ti_acc1, ti_acc5 = self.clip_loss(proj_img_emb_1, proj_text_emb)
        ...
        if unified_train:
            ...
            if text_aug:
                ...
                text_contras_loss = self.covar_loss(project_t_contras_emb_1, project_t_contras_emb_2)
                return {
                        'clip_loss': clip_loss,
                        'slip_loss': slip_loss,
                        'text_d_loss': text_contras_loss,
